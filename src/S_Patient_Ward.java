...
public class S_Patient_Ward extends JFrame {
...
//-----------------------------------------------------------------------------------------------------------------------------
        ...
        Object[] columns = {"Patients ID"," Full Name","Gender","Telephone","Age","Type","Date Time"};
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------
        addbu.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                ...
                fileChooser.setSelectedFile(new File("Patient Details.pdf"));
...
//------------------------------------------------------------------------------------------------------------------------------
        ...
        a.setIcon(new ImageIcon(getClass().getResource("img/S_Patient_Ward.png")));
        add(a);
//------------------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    private static void addImageToBackground(PdfWriter writer, String imagePath, Document document) {
        try {
            // Load image
            Image image = Image.getInstance(S_Patient_Ward.class.getResource(imagePath));
...
//            cb.showText(dname);
    ...
    private static void addTableHeader(PdfPTable table) {
        String[] headers = {"Patients ID", "Full Name", "Gender", "Telephone", "Age", "Type", "Date"};
    ...
    private static void fetchDataAndAddToTable(PdfPTable table) {
        ...
        try (Connection connection = DriverManager.getConnection(url, user, password);
             ...
             ResultSet resultSet = statement.executeQuery("SELECT * FROM patient_update INNER JOIN patient_register ON patient_update.NIC_Number = patient_register.NIC_Number WHERE patient_update.status = '"+ward.getText()+ "' AND patient_update.ward_no = '"+userward.getText()+ "'")) {
